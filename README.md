...
obsidian, raycast, spotify, zed, font-blex-mono-nerd-font, font-jetbrains-mono-nerd-font
...
- Obsidian preferences include graph view settings and Excalidraw vault-library and Markdown image settings.
- Herdr configuration uses `Ctrl+Alt+hjkl` for pane focus. Run `herdr config check` after edits. Neovim enables Zellij navigation only inside Zellij.
